...
export default async function EditProductPage({
  ...
  const PUBLISH_BLOCKED_MESSAGES: Record<string, string> = {
    ...
    // 缺少场景图,不可发布 (Jym, hard gate) — a white-background cutout alone
    // is a half-finished listing; "See it, buy it" needs the product shown in
    // a room. Scenes are generated by the Bulk AI "Generate scene images" flow.
    scene:
      "缺少场景图,不可发布。回产品列表用 Bulk AI 的 “Generate scene images” 为它生成场景图后再发布 —— 白底抠图上前台是半成品。",
    glb:
